...
export const ModuleControl = ({
  ...
  const getFilteredModules = () => {
    ...
    const matchesSearch = (moduleId, moduleStatus) => {
      ...
      const moduleIdMatch =
        numericIdText.includes(normalizedSearch) ||
        displayId.includes(normalizedSearch)
      const siteNameMatch =
        moduleStatus?.siteName &&
        moduleStatus.siteName.toLowerCase().includes(normalizedSearch)
    ...
    if (isDevelopment && !modules.some((module) => module.id === 0)) {
      ...
      if (normalizedSearch) {
        ...
        const moduleIdMatch =
          '0'.includes(normalizedSearch) || displayId.includes(normalizedSearch)
        const siteNameMatch = dummyStatus.siteName
          .toLowerCase()
