...
export const ModuleControl = ({ mqttClient, subscribedTopics, filter, setFilter, searchTerm, setSearchTerm, onGlobalCommand }) => {
    ...
    const handleGlobalCommand = (command) => {
        const connectedModules = Object.keys(moduleStatuses).filter((id) => moduleStatuses[id].isConnected);
                ...
                <div className="modules-table-scroll-container">
                    <div className="modules-table-content">
                        <div className="modules-table-header">
                            ...
                            <div className="modules-table-scrollable">
                                <div>용량</div>
                                <div>촬영진행</div>
                                <div>누락</div>
                                <div>마지막 촬영</div>
                        ...
                        {filteredModules.length === 0 ? (
                            ...
                            filteredModules.map(({ id, status, settings }) => (
                                <CameraModuleRow key={id} moduleId={id} status={status} availableSettings={settings} onCommand={handleCommand} onLoadSettings={handleLoadSettings} />
                            ))
                        )}
                    </div>
                </div>
