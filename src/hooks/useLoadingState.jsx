...
export const useLoadingState = (options = {}) => {
    ...
    const startLoading = useCallback((key, message = null) => {
        ...
        if (timeout && timeout > 0) {
            const timeoutId = setTimeout(() => {
                setLoadingStates(prev => {
                    if (prev[key]?.loading) {
                        const timeoutError = new Error(`작업이 시간 초과되었습니다 (${timeout}ms)`);
                        if (onTimeout) {
     ...
     */
    const errorLoading = useCallback((key, error) => {
        // 타임아웃 클리어
        const timeoutId = timeoutRefs.current.get(key);
        if (timeoutId) {
            clearTimeout(timeoutId);
     ...
     */
    const withLoading = useCallback(async (key, promiseOrFunction, message = null) => {
        try {
     ...
     */
    const isLoading = useCallback((keys) => {
    ...
    return {
        ...
        loadingCount: Object.values(loadingStates).filter(state => state.loading).length
    };
};